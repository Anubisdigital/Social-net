/**
 * index Social Network - Backend Server
...
app.use(cors({
    origin: function(origin, callback) {
        ...
        const allowedOrigins = [
            'http://localhost:3000',
            'http://localhost:5173',
            'https://pleasedont.onrender.com',
            'https://pleasedont.netlify.app',
            'https://index-social.netlify.app'
        ];
...
const apiLimiter = rateLimit({
    ...
    max: process.env.NODE_ENV === 'production' ? 200 : 100, // Increased for production
...
// MongoDB connection string - Render provides MONGODB_URI
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/index';
...
passport.deserializeUser(async (id, done) => {
    try {
        ...
        if (mongoose.connection.readyState === 1) {
            ...
            user = users.find(u => u.id === id || u._id === id);
        }
...
if (mongoose.connection.readyState === 1) {
    ...
    passport.use(new LocalStrategy({
        ...
        try {
            const User = mongoose.model('User');
            const user = await User.findOne({ email: email.toLowerCase(), provider: 'email' }).select('+password');
...
if (process.env.GOOGLE_CLIENT_ID && process.env.GOOGLE_CLIENT_SECRET) {
    ...
    // Build callback URL for Render
    const googleCallbackURL = process.env.GOOGLE_CALLBACK_URL || 
        `${process.env.RENDER_EXTERNAL_URL || 'http://localhost:' + PORT}/auth/google/callback`;
...
if (process.env.GITHUB_CLIENT_ID && process.env.GITHUB_CLIENT_SECRET) {
    ...
    const githubCallbackURL = process.env.GITHUB_CALLBACK_URL || 
        `${process.env.RENDER_EXTERNAL_URL || 'http://localhost:' + PORT}/auth/github/callback`;
...
if (process.env.FACEBOOK_APP_ID && process.env.FACEBOOK_APP_SECRET) {
    ...
    const facebookCallbackURL = process.env.FACEBOOK_CALLBACK_URL || 
        `${process.env.RENDER_EXTERNAL_URL || 'http://localhost:' + PORT}/auth/facebook/callback`;
...
app.get('/auth/github/callback',
    ...
    (req, res) => {
        ...
        // Redirect to frontend with success message
        const frontendURL = process.env.FRONTEND_URL || 'http://localhost:3000';
        res.redirect(`${frontendURL}/?auth=success&provider=github`);
    }
);
...
app.get('/auth/facebook/callback',
    ...
    (req, res) => {
        ...
        // Redirect to frontend with success message
        const frontendURL = process.env.FRONTEND_URL || 'http://localhost:3000';
        res.redirect(`${frontendURL}/?auth=success&provider=facebook`);
    }
);
...
app.get('/auth/test', (req, res) => {
    res.json({
        ...
        facebookConfigured: !!(process.env.FACEBOOK_APP_ID && process.env.FACEBOOK_APP_SECRET),
        callbackURLs: {
            google: process.env.GOOGLE_CALLBACK_URL || `${process.env.RENDER_EXTERNAL_URL || 'http://localhost:' + PORT}/auth/google/callback`,
            github: process.env.GITHUB_CALLBACK_URL || `${process.env.RENDER_EXTERNAL_URL || 'http://localhost:' + PORT}/auth/github/callback`,
            facebook: process.env.FACEBOOK_CALLBACK_URL || `${process.env.RENDER_EXTERNAL_URL || 'http://localhost:' + PORT}/auth/facebook/callback`
        }
    });
});
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    if (providers.length > 0) {
        ...
        if (process.env.GOOGLE_CLIENT_ID) {
            console.log(`   Google: ${process.env.GOOGLE_CALLBACK_URL || `${process.env.RENDER_EXTERNAL_URL || 'http://localhost:' + PORT}/auth/google/callback`}`);
        }
        if (process.env.GITHUB_CLIENT_ID) {
            console.log(`   GitHub: ${process.env.GITHUB_CALLBACK_URL || `${process.env.RENDER_EXTERNAL_URL || 'http://localhost:' + PORT}/auth/github/callback`}`);
        }
        if (process.env.FACEBOOK_APP_ID) {
            console.log(`   Facebook: ${process.env.FACEBOOK_CALLBACK_URL || `${process.env.RENDER_EXTERNAL_URL || 'http://localhost:' + PORT}/auth/facebook/callback`}`);
        }
    } else {
    ...
    console.log(`   Auth test: GET /auth/test`);
    console.log(`   User count: GET /api/users/count`);
